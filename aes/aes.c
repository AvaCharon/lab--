...
//请补充代码
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            temp[i][j]=GFMul(colM[i][0],array[0][j])^GFMul(colM[i][1],array[1][j])^GFMul(colM[i][2],array[2][j])^GFMul(colM[i][3],array[3][j]);
        }
    }
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
...
//请补充代码
    ...
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            temp[i][j]=GFMul(deColM[i][0],array[0][j])^GFMul(deColM[i][1],array[1][j])^GFMul(deColM[i][2],array[2][j])^GFMul(deColM[i][3],array[3][j]);
        }
    }
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
...
void addRoundKey(int array[4][4], int round)
{
    ...
    for(int i=0;i<4;i++){
        //这里是以行排列的
        splitIntToArray(w[round*4+i],temp[i]);
    }
    //请补充代码
    for(int i=0;i<4;i++){
        for(int j=0;j<4;j++){
            array[i][j]=array[i][j]^temp[j][i];
        }
    }
}
